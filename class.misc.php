...
class misc {
	...
	*/
	 ...
	 */
	public static function getCookie($bduss) {
//		global $m;
//		if (empty($pid)) {
//			return false;
//		}
...
//		}
	 ...
	 */
	public static function scanTiebaByPid($cma) {
		$uid    = $cma['uid'];
		$ubduss = $cma['bduss'];
		$pid    = $cma['id'];
		$n3     = 1;
		...
		while(true) {
			...
			foreach ($list[3] as $v) {
				...
				print_r($v."吧 添加任务到签到队列");
				echo("\n");
				
				try{
					//新建队列
					$queue = new SaeTaskQueue('tieba_sign');
					
					//批量添加任务
					$array = array();
					$array[] = array('url'=>"/public/sign/tiebaMain.php", "postdata"=>"kw=".$v, "prior"=>false,"options" => array('delay' => '60'));
					$queue->addTask($array);
